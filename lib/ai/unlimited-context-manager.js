...
class UnlimitedContextManager extends EventEmitter {
    ...
    /**
     * Process unlimited context - alias for backward compatibility
     * @param {Object} request - Context processing request
     * @returns {Promise<Object>} Processed context result
     */
    async processUnlimitedContext(request = {}) {
        const position = request.position || request.uri || {};
        const language = request.language || 'javascript';
        const options = { ...request, unlimitedProcessing: true };

        const result = await this.assembleUnlimitedContext(position, language, options);
    ...
    async analyzeCurrentFile(position, language) {
        // Handle undefined or null position gracefully
        const safePosition = position || {};
        const filePath = safePosition.uri || safePosition.filePath || 'current.js';
    ...
    async analyzeRelatedFiles(position, language) {
        // Handle undefined or null position gracefully
        const safePosition = position || {};
        const basePath = safePosition.uri || safePosition.filePath || 'related';
        return [
    ...
    buildDependencyGraph(internalDependencies) {
        ...
        const depArray = Array.from(internalDependencies.values()).flat();
        const nodes = depArray.map((dep, index) => ({ id: index, name: dep }));
        const edges = depArray.map((dep, index) => ({ from: index, to: (index + 1) % depArray.length }));
        return { nodes, edges, dependencies: depArray };
    }
    ...
    detectUnusedDependencies(externalDependencies) {
        ...
        // Convert Map to array of values for processing
        const depArray = Array.from(externalDependencies.values()).flat();
        return depArray.filter((dep, index) => index % 3 === 0)
    ...
    extractSemanticRelations(symbolExtraction, dependencyMapping) {
        ...
        symbolExtraction.functions.forEach(func => {
            // Handle the internal dependencies properly - it might be a Map or nested structure
            const internalDeps = dependencyMapping.internalDependencies ?
                Array.from(dependencyMapping.internalDependencies.values()).flat() :
                (dependencyMapping.internal || []);
    ...
    analyzeContextualMeaning(symbolExtraction, dependencyMapping, language) {
        ...
        const meaning = `${language} code with ${symbolExtraction.functions.length} functions and ${internalDeps.length} internal dependencies`;
        return {
            meaning,
            language: language,
            context: {
