...
class ModelSelector {
  ...
  selectModel(request) {
    ...
    try {
      ...
      console.debug(`Model selection for ${requestType} request: ${language}, ${tokenCount} tokens, priority: ${priority}`);

      // Apply user-defined complexity override if specified
      if (complexity !== null) {
        console.debug(`Using user-defined complexity: ${complexity}`);
      }
      ...
      const contextSize = context.totalTokens || tokenCount;
      const complexityAnalysis = this.analyzeComplexity({ ...request, codeComplexity, contextSize });
  ...
  /**
   * Analyze code complexity based on content and language
   * @private
   */
  analyzeCodeComplexity(code, language) {
    const languagePatterns = {
      javascript: [/async\s+function/, /Promise/, /=>/, /class\s+\w+/],
      python: [/def\s+\w+/, /class\s+\w+/, /import\s+/, /async\s+def/],
      shell: [/function\s+\w+/, /for\s+\w+\s+in/, /if\s+\[/, /case\s+/]
    };

    const patterns = languagePatterns[language] || languagePatterns.javascript;
    const complexityScore = patterns.reduce((score, pattern) => {
    ...
    return {
      ...
      length: code.length,
      language: language,
      hasComplexPatterns: complexityScore > 2
    };
  }
