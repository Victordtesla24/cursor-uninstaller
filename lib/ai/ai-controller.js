...
import BasicModelSelector from './model-selector.js';
...
class AIController {
  ...
  async executeWithProvider(provider, request, modelSelection, context) {
    ...
    // Allow mock responses when clients aren't configured (for testing)
    // The clients will return mock data when no API key is provided
  ...
  /**
   * Generate code completion (specific method for AISystem compatibility)
   * @param {string} code - Code to complete
   * @param {string} language - Programming language
   * @param {Object} position - Cursor position
   * @returns {Promise<Object>} Completion result
   */
  async generateCompletion(code, language, position = {}) {
    const request = {
      type: 'completion',
      code,
      language,
    ...
    if (result.success) {
      return {
        ...
        text: result.text,
        confidence: 0.8,
        model: result.metadata?.model || 'claude-3.7-sonnet-thinking',
  ...
  /**
   * Execute instruction/refactoring request (specific method for AISystem compatibility)
   * @param {string} instruction - Instruction to execute
   * @param {string} language - Programming language
   * @param {Object} selection - Code selection
   * @returns {Promise<Object>} Instruction result
   */
  async executeInstruction(instruction, language = 'javascript', selection = null) {
    const request = {
      type: 'instruction',
      instruction,
      language,
    ...
    if (result.success) {
      return {
        result: result.text,
        text: result.text,
        edits: this._parseEditsFromText(result.text),
        explanation: 'Instruction executed successfully',
        model: result.metadata?.model || 'claude-4-sonnet-thinking',
        metadata: result.metadata
      };
    } else {
      throw new Error(result.error);
  ...
  /**
   * Parse edits from instruction result text
   * @param {string} text - Result text
   * @returns {Array} Array of edit objects
   */
  _parseEditsFromText(text) {
    // Simple edit parsing - in production this would be more sophisticated
    if (!text || typeof text !== 'string') {
      return [];
    }

    // Look for code blocks or structured edits
