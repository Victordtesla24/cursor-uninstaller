...
class PythonAdapter extends BaseLanguageAdapter {
    ...
    async setupLSP() {
        try {
            if (!this.config.enablePylance) {
                this.pylanceClient = null;
                return;
            }

            // Detect and activate virtual environment first
            ...
            if (pythonVersion) {
                // Mock Pylance LSP client - would connect to real Pylance server in production
                this.pylanceClient = {
                    connected: true,
                    version: '2023.12.1',
                    ...
                    async getCompletions(file, position) {
                        // Would call real Pylance LSP in production
                        console.debug(`Pylance completions for ${file} at ${JSON.stringify(position)}`);
                        return [];
                    },
                    async getDiagnostics(file, content) {
                        // Would call real Pylance LSP in production
                        console.debug(`Pylance diagnostics for ${file} (${content.length} chars)`);
                        return [];
                    },
                    async getDefinition(file, position) {
                        // Would call real Pylance LSP in production
                        console.debug(`Pylance definition for ${file} at ${JSON.stringify(position)}`);
                        return null;
                    },
                    async getSignatureHelp(file, position) {
                        // Would call real Pylance LSP in production
                        console.debug(`Pylance signature help for ${file} at ${JSON.stringify(position)}`);
                        return null;
                    }
                };

                if (!this.config.quietMode) {
                    console.log(`✓ Pylance LSP client initialized (Python ${pythonVersion})`);
                }
            }
     ...
     */
    async performFormatting(content, _options = {}) {
    ...
    async checkPythonVersion() {
        try {
            const result = await this.executePython('import sys; print(sys.version)', [], { timeout: 5000 });
            
            if (result.stdout) {
                // Extract version number from output like "3.11.0 (main, Oct 24 2022, ...)"
            ...
            try {
                const result = await this.executePython('import sys; print(".".join(map(str, sys.version_info[:3])))', [], { timeout: 3000 });
                return result.stdout.trim() || null;
            } catch (_fallbackError) {
                console.warn(`Python version check failed: ${error.message}`);
                return null;
            }
        }
    }
