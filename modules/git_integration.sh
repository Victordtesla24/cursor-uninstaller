...
display_git_repository_info() {
    ...
    if ! git rev-parse --git-dir >/dev/null 2>&1; then
        log_with_level "ERROR" "✗ Current directory is not a git repository"
        return 1
    fi
    ...
    if command -v git >/dev/null 2>&1; then
        ...
        log_with_level "SUCCESS" "✓ Git is installed: $git_version"
    else
        log_with_level "ERROR" "✗ Git is not installed on this system"
        return 1
    fi
...
perform_git_commit_and_push() {
    ...
    if [[ -z "$status_output" ]]; then
        log_with_level "INFO" "✓ No changes to commit - working directory is clean"
        return 0
    fi
    ...
    if [[ ${#filtered_files[@]} -eq 0 ]]; then
        log_with_level "INFO" "✓ No relevant changes to commit after filtering generated files"
        return 0
    fi
    ...
    for file in "${filtered_files[@]}"; do
        if ! git add "$file" 2>/dev/null; then
            log_with_level "WARNING" "⚠ Could not add file: $file"
            add_success=false
        fi
    done

    if [[ "$add_success" == "true" ]]; then
        log_with_level "SUCCESS" "✓ Relevant changes staged successfully"
    else
        log_with_level "WARNING" "⚠ Some files could not be staged, continuing with available files"
    fi
    ...
    if ! git diff --cached --quiet 2>/dev/null; then
        ...
        if git commit -m "$commit_message"; then
            log_with_level "SUCCESS" "✓ Changes committed successfully"
        else
            log_with_level "ERROR" "✗ Failed to commit changes"
            return 1
        fi
        ...
        if timeout "${git_push_timeout}" git push origin "$current_branch"; then
            log_with_level "SUCCESS" "✓ Changes pushed to remote repository successfully"
        else
            local push_exit_code=$?
            log_with_level "WARNING" "⚠ Failed to push to remote (exit code: $push_exit_code)"
        ...
        if [[ -z "$final_status" ]]; then
            log_with_level "SUCCESS" "✓ Working directory is now clean"
        else
        ...
        log_with_level "INFO" "✓ No staged changes to commit"
    fi
